/**
 * 兼容 https://github.com/remaxjs/remax/blob/4cb412ec28bd4828f2cc7fc9807d3a2995ae44e8/packages/remax-cli/src/index.ts#L7-L9
 *
 * 3.0 可以干掉
 */
import { Options } from '@remax/types';
import API from './API';
export declare function buildMini(api: API, options: Options): import("webpack").Compiler;
export declare function buildWeb(api: API, options: Options): import("webpack").Compiler;
export { getDefaultOptions } from './defaultOptions';
