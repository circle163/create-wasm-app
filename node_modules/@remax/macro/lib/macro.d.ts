...
import { pageEvents } from './usePageEvent';
import { appEvents } from './useAppEvent';
declare type PageEventName = 'onLoad' | 'onShow' | 'onHide' | 'onReady' | 'onPullDownRefresh' | 'onReachBottom' | 'onPageScroll' | 'onShareAppMessage' | 'onTitleClick' | 'onOptionMenuClick' | 'onPopMenuClick' | 'onPullIntercept' | 'onBack' | 'onKeyboardHeight' | 'onTabItemTap' | 'beforeTabItemTap' | 'onResize' | 'onUnload';
declare type AppEventName = 'onLaunch' | 'onShow' | 'onHide' | 'onError' | 'onShareAppMessage' | 'onPageNotFound' | 'onUnhandledRejection' | 'onThemeChange';
